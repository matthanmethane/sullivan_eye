...
class Scanner():
    def __init__(self, bookname):
        ...
        self.page_no = 1
    def open_camera(self,speaker, flag=False):
        ...
        cv.destroyAllWindows()
        #Test
        img_url = rf'img/covid1.png'
        show_url = rf'img/sample1.png'
        if flag:
            img_url = rf'img/covid2.png'
        frame = cv.imread(show_url)
        demo = cv.imread(img_url)
        gray = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)
        demo_gray = cv.cvtColor(demo,cv.COLOR_BGR2GRAY)
        crop = gray [0:1080,500:1200]
        cv.imshow("Scanned Image",crop)
        cv.waitKey(1000) 
        cv.destroyAllWindows()
        text = pytesseract.image_to_string(demo_gray, lang='eng', config='--psm 3')
        print(text)
        cv.imshow("Scanned Image",crop)
        ...
        while(True):    
            ...
            if cv.waitKey(0) == ord('r'):
                ...
                speaker.say("Saving")
                speaker.run()
                with open(img_url.replace("img","text").replace("png","txt"),'w') as f:
                    f.write(text)
                speaker.say("Document saved")
                speaker.run()
                cv.destroyAllWindows()
                self.open_camera(speaker, True)
                return text
